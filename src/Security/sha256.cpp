...
static uint8_t *create_padded_buffer(const uint8_t *buffer, size_t buffer_size, size_t *padded_size)
{
    ...
    *padded_size = (*padded_size + 63) & ~63; // Round up to the next multiple of 64
    ...
    uint8_t *padded_buffer = (uint8_t *)operator new(*padded_size);
    if (!padded_buffer)
    {
        *padded_size = 0;
        return (NULL);
    ...
    std::memcpy(padded_buffer, buffer, buffer_size);
    padded_buffer[buffer_size] = 0x80;
    std::memset(padded_buffer + buffer_size + 1, 0, (*padded_size - buffer_size - 9));
...
{
    size_t padded_size;
    uint8_t *padded_buffer = create_padded_buffer(reinterpret_cast<const uint8_t *>(buffer), buffer_size, &padded_size);
    ...
    for (int i = 0; i < 32; i++) {
		std::cout << std::hex << (int)sha256_key[i];        
	}
	std::cout << std::endl;
	operator delete(padded_buffer); 
    return (0);
}
